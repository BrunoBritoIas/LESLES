...
public class SuplementoDAO extends AbstractJdbcDAO {
	...
	public void salvar(EntidadeDominio entidade) throws SQLException {
		...
		try {
			...
			sql.append("INSERT INTO Suplemento(nome, marca, categoria, descricao, peso, preco, proteina, carboidratos, gordura, calorias, rating, stats, validade)");
			sql.append("VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");		
	...
	public void alterar(EntidadeDominio entidade) throws SQLException {
		...
		Suplementos suplemento = (Suplementos)entidade;
				...
				try {
					connection.setAutoCommit(false);
					StringBuilder sql = new StringBuilder();
					sql.append("UPDATE Suplemento SET nome=?, marca=?, categoria=?, descricao=?, peso=?, preco=?, proteina=?, carboidratos=?, gordura=?, calorias=?, stats=?,validade=?"
					+  " WHERE Id_suplemento=?");
